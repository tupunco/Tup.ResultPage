...
namespace Tup.ResultPage.Utils
{
    ...
    public static class ThrowHelper
    {
        ...
        {
            if (!ArrayUtil.Exists(boundArray, delegate(int x) { return x == argument; }))
                throw new ArgumentOutOfRangeException(name, string.Format("[{0}]", string.Join("],[", ArrayUtil.ConvertAll(boundArray, new Converter<int, string>(delegate(int x) { return x.ToString(); })))));
        }
        /// <summary>
        /// 超界    判断
        /// </summary>
        ...
        {
            if (!ArrayUtil.Exists(boundArray, x => x == argument))
